...
const apiKeyAuthSnapshotVersion = 14 // v14: include multi-tier routing, group peak rate, and group video pricing fields
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		PreferredTierKey:    apiKey.PreferredTierKey,
		TierFallbackEnabled: apiKey.TierFallbackEnabled,
		TierFallbackPolicy:  normalizeJSONMap(apiKey.TierFallbackPolicy),
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		PreferredTierKey:    snapshot.PreferredTierKey,
		TierFallbackEnabled: snapshot.TierFallbackEnabled,
		TierFallbackPolicy:  normalizeJSONMap(snapshot.TierFallbackPolicy),
